...
import { ImageFile, FsNgAlainImagePickerComponent } from '@fs-tw/theme-alain/shared';
...
export class PickCoverImageComponent implements OnInit, OnChanges {
  ...
  ngOnChanges() {
    if (this.coverImageMediaId) {
        let url = `${this.environmentService.getApiUrl('cms-kit')}/api/cms-kit/media/${this.coverImageMediaId}`
