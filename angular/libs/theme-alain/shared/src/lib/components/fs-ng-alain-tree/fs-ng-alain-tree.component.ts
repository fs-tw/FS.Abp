...
export class FsNgAlainTreeComponent implements OnInit {
  ...
  /** 初始化 Tree 所需要的資料 */
  private initTreeDatas(tree:any){
    tree = tree.map(function(x:any){
  ...
  public getChecked(type: string = 'id') {
    var checkedDatas = this.nodes2Array(_.cloneDeep(this.treeOptions))
                      .filter((x:any) => x.isChecked && x.key != 0)
                      .map((x:any) => x.key);
    
    var checkedTreeDatas = _.cloneDeep(this.tree).filter((x:any) => checkedDatas.includes(x.id));
