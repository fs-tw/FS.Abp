...
(function () {
    ...
    function loginUserInternal(tenantId, callback) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    ...
                    var expireDate = new Date(Date.now() + (result.token.expireInSeconds * 1000));
                    abp.auth.setToken(result.token.accessToken, expireDate);
                    callback();   
                } else {
                    alert('Login failed !');
                }
        ...
        xhr.open('POST', '/api/common/tokenAuth/authenticate', true);
        if(!!tenantId)
    ...
    abp.swagger.login = function (callback) {
        ...
        if (tenancyName) {
            ...
            xhrTenancyName.onreadystatechange = function () {
                if (xhrTenancyName.readyState === XMLHttpRequest.DONE && xhrTenancyName.status === 200) {
                    ...
                    if (result.state === 1) { // Tenant exists and active.
                        loginUserInternal(result.tenant.id, callback); // Login for tenant    
                    } else {
            ...
            xhrTenancyName.open('POST', '/api/common/tokenAuth/isTenantAvailable', true);
