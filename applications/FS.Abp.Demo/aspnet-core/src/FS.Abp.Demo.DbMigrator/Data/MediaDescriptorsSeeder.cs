...
namespace FS.Abp.Demo.DbMigrator.Data
{
    ...
    public class MediaDescriptorsSeeder : FS.Abp.Data.Seeder<MediaDescriptorsSeederOptions>, ITransientDependency
    {
        ...
        {
            List<MediaDescriptor> mediaDescriptorList = new List<MediaDescriptor>();
            List<AttachmentMedia> attachmentMediaList = new List<AttachmentMedia>();

            await processMediaDescriptorAsync();
            await processAttachmentMediaAsync();

            await MediaDescriptorRepository.InsertManyAsync(mediaDescriptorList);
            await MediaDescriptorsStore.AttachmentMedia.InsertManyAsync(attachmentMediaList, true);

            async Task processMediaDescriptorAsync()
            {
                ...
                foreach (var file in VirtualFileProvider.GetDirectoryContents(Options.MediaDirectoryPath))
                {
                    ...
                    mediaDescriptorList.Add(img);
                }
            }

            async Task processAttachmentMediaAsync() 
            {
                var existedDatasCount = await MediaDescriptorsStore.AttachmentMedia.GetCountAsync();
                if (existedDatasCount > 0)
                    return;
                
                var attachmentMedias = VirtualFileNpoiReader.Read<MediaDescriptorsInfo>(Options.FileName, Options.SheetName);
                
                foreach (var item in attachmentMedias)
                {
                    Page page = await PageRepository.GetBySlugAsync(item.PageSlug);
                    MediaDescriptor mediaDescriptor = mediaDescriptorList.Where(x => x.Name == item.FileName).Single();
